...
export const getStripeClientSecretKey = (selectedVehicleUuid: string) => {
    ...
    if (!stripeSecretKey || stripeSecretKey.vehicleUuid !== selectedVehicleUuid) {
        // if not saved Key or saved Key UUID not Same with Current Fetch Again
        return;
    } else {
        //Already saved secret key with Same UUID (route / vehicle hasn't change, hence the price is same)
