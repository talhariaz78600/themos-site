...
const Footer: React.FC = () => {
  const renderWidgetMenuItem = (menu: WidgetFooterMenu, index: number) => {
    return (
      <div key={index} className="text-sm ref-footer">
        <p className="font-semibold  dark:text-neutral-200 text-neutral-200 leading-relaxed text-[36px]">
          {menu.title}
        </p>
        <ul className="mt-5 space-y-4">
          {menu.menus.map((item, index) => (
            <li key={index}>
              <Link className="text-neutral-300 dark:text-neutral-300 hover:text-black dark:hover:text-white hover:text-white" href={item.href}>    
                  {item.label}
              </Link>
            </li>
          ))}
  ...
  return (
    <div className="nc-Footer relative py-24 lg:py-16 border-t border-neutral-200 dark:border-neutral-700  md:px-1 lg:px-2">
      ...
      <p className="text-center text-white pt-10 font-bold text-[42px] md:text-[46px] leading-relaxed">
        Member Of The Greek Tourism Organization
      </p>
      <p className="text-center text-white font-bold text-[42px] md:text-[46px] leading-relaxed">Registration Number: O206E81000524201</p>
    </div>
  );
};
