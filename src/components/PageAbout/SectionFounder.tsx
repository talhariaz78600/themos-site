      ...
      <div className="grid sm:grid-cols-2 gap-x-5 gap-y-8 lg:grid-cols-4 xl:gap-x-8">
        {FOUNDER_DEMO.map((item) => (
          <div key={item.id} className="max-w-sm">
            ...
            <h3 className="text-lg font-semibold text-neutral-200 mt-4 md:text-xl dark:text-neutral-200">
              {item.name}
            </h3>
            <span className="block text-sm text-neutral-400 sm:text-base dark:text-neutral-400">
              {item.job}
            </span>
          </div>
        ))}
